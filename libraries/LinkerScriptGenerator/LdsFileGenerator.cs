...
namespace LinkerScriptGenerator
{
    public class LdsFileGenerator : IDisposable
    {
        ...
        public void GenerateLdsFile(StreamWriter sw, Dictionary<string, string> memorySubstitutionMap, IEnumerable<string> extraLines = null)
        {
            ...
            OutputSectionDefinitions(sw, memorySubstitutionMap);
        ...
        private void OutputSectionDefinitions(StreamWriter sw, Dictionary<string, string> memorySubstitutionMap)
        {
            ...
            for (int i = 0; i < _ScriptTemplate.Sections.Count; i++)
            {
                ...
                OutputSectionDefinition(sw, "\t", section, memorySubstitutionMap);
        ...
        void OutputSectionDefinition(StreamWriter sw, string padding, Section section, Dictionary<string, string> memorySubstitutionMap)
        {
            string nameAsID = section.Name.TrimStart('.').Replace('.', '_');

            Memory targetMemory;
            if (_Memories.TryGetValue(section.TargetMemory, out targetMemory))
            {
                //Found target memory.
            }
            else if (section.TargetMemory == "SRAM")
                targetMemory = _MainRAM;
            else if (section.TargetMemory == "FLASH")
                targetMemory = _MainFLASH;
            else
                throw new Exception($"{section.TargetMemory} not defined in {_MemoryTemplate.DeviceName}");

            if (memorySubstitutionMap != null && memorySubstitutionMap.TryGetValue(targetMemory.Name, out var newName))
                targetMemory = _Memories[newName];

            string loadedMemoryName = null;
            if ((section.Flags & SectionFlags.InitializerInMainMemory) != SectionFlags.None)
            {
                loadedMemoryName = _MainFLASH.Name;
                if (memorySubstitutionMap != null && memorySubstitutionMap.TryGetValue(loadedMemoryName, out var substitute))
                    loadedMemoryName = substitute;
            }
            ...
            if (loadedMemoryName == null || loadedMemoryName == targetMemory.Name)
                sw.WriteLine(padding + "}" + " > " + targetMemory.Name);
            else
                sw.WriteLine(padding + "}" + " > " + targetMemory.Name + " AT >" + loadedMemoryName);
        ...
        public Dictionary<string, string> CreateDefaultFLASHToRAMSubstitutionMap()
        {
            return new Dictionary<string, string> { { _MainFLASH.Name, _MainRAM.Name } };
        }
